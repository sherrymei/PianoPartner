...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST['feedback'])) {
    ...
    if ($stmt0 = $conn->prepare("INSERT INTO Feedback (UserID,FeedbackMsg) VALUES (?,?)")){
      ...
      $stmt0 ->execute();
      $feedback_message = "Feedback Accepted!";
      $stmt0 ->close();
    }
    else {
      echo $stmt->error;
      ...
      <div class="text-center">
        ...
        if ($stmt1 = $conn->prepare("SELECT Users.UserID, Users.StatusMsg, Users.Tempo, Users.Recording, Payment.Pages, Payment.Class, Payment.Amount FROM Users LEFT JOIN Payment ON Users.OrderNumber = Payment.OrderNumber WHERE Users.OrderNumber = ?;")){
          ...
          switch ($status_msg) {
            ...
            <form>
              <label for="check"><input id="check" type="checkbox">I agree to the <a href="terms_and_conditions">Terms and Conditons</a></label>
            </form>
            ...
            case "Status4":
            ?>
            <p>Thank you for your payment. Accompanist is recording your piece. Please wait up to 1 week for audio recordings and 2 weeks for video recordings. </p>
            <?php
            break;
            case "Status5":
            ?>
            ...
            <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
              ...
              <div class="row">
                <div class="col-3"></div>
              <div class="col-6">
                <input type="hidden" name="order" value="<?php echo $order_num;?>"/>
                <textarea class="form-control" rows="6" name="feedback"></textarea><br>
            ...
            <p>Click below to fill out order form.</p>
